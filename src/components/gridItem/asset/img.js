import Image from "next/image";
import Link from "next/link";
import dynamic from "next/dynamic";

// Sanity
import { urlFor } from "../../../../lib/sanity";
// Assets
import PlaceholderImg from "../../../../public/placeholder.jpeg";
// Components
const SocialShareOverlay = dynamic(() =>
  import("../../../CMSComponents/carousel/socialShareOverlay")
);
// import Caption from '../../CMSComponents/shared/caption'

const Img = ({ data, setFilters }) => {
  if (data?._type === "caseStudy") {
    return (
      <Link href={`${data?.asset?.path ?? "/"}`} prefetch={false}>
        <a className='transition duration-150 shadow-lg hover:shadow-x relative overflow-hidden group'>
          <div className='transition duration-300 ease-in-out transform hover:scale-110'>
            <Image
              src={data?.asset?.featuredImage?.asset?.url ?? PlaceholderImg}
              layout='responsive'
              objectFit='cover'
              alt={data?.asset.alt}
              height={
                data?.asset?.featuredImage?.asset?.metadata?.dimensions
                  ?.height ?? "100%"
              }
              width={
                data?.asset?.featuredImage?.asset?.metadata?.dimensions
                  ?.width ?? "100%"
              }
              placeholder='blur'
              blurDataURL='data:image/jpeg;base64,'
            />
          </div>
          <SocialShareOverlay
            title={data?.title ?? data?.asset?.name}
            // showShare='true'
            slug={data?.asset?.path ?? "/"}
          />
        </a>
      </Link>
    );
  } else if (data?._type === "asset") {
    return (
      <Link href={`/designs/${data?.slug?.current ?? "/"}`} prefetch={false}>
        <a className='transition duration-150 shadow-lg hover:shadow-x relative overflow-hidden group'>
          <div className='transition duration-300 ease-in-out transform hover:scale-110'>
            <Image
              src={data?.featuredImage?.asset?.url ?? PlaceholderImg}
              layout='responsive'
              objectFit='cover'
              alt={data?.featuredImage?.alt}
              height={
                data?.featuredImage?.asset?.metadata?.dimensions?.height ??
                "100%"
              }
              width={
                data?.featuredImage?.asset?.metadata?.dimensions?.width ??
                "100%"
              }
              placeholder='blur'
              blurDataURL='data:image/jpeg;base64,'
            />
          </div>
          <SocialShareOverlay
            title={data?.name}
            //   showShare='true'
            slug={`/designs/${data?.slug?.current}`}
          />
        </a>
      </Link>
    );
  } else if (data?._type === "application") {
    return (
      <div
        className='transition duration-150 shadow-lg hover:shadow-x relative overflow-hidden group cursor-pointer'
        onClick={() => {
          return setFilters([{ type: data?._type, name: data?.name }]);
        }}
      >
        <div className='transition duration-300 ease-in-out transform hover:scale-110'>
          <Image
            src={data?.featuredImage?.asset?.url ?? PlaceholderImg}
            layout='responsive'
            objectFit='cover'
            alt={data?.featuredImage?.alt}
            height={
              data?.featuredImage?.asset?.metadata?.dimensions?.height ?? "100%"
            }
            width={
              data?.featuredImage?.asset?.metadata?.dimensions?.width ?? "100%"
            }
            placeholder='blur'
            blurDataURL='data:image/jpeg;base64,'
          />
        </div>
        <SocialShareOverlay
          title={data?.name}
          // showShare='true'
          slug={`/applications/${data?.slug?.current}`}
        />
      </div>
    );
  } else if (data?._type === "specialtie") {
    return (
      <Link href={`/specialties/${data?.slug?.current}`} prefetch={false}>
        <a className='transition duration-150 shadow-lg hover:shadow-x relative overflow-hidden group'>
          <div className='transition duration-300 ease-in-out transform hover:scale-110'>
            <Image
              src={data?.featuredImage?.asset?.url ?? PlaceholderImg}
              layout='responsive'
              objectFit='cover'
              alt={data?.featuredImage?.alt}
              height={
                data?.featuredImage?.asset?.metadata?.dimensions?.height ??
                "100%"
              }
              width={
                data?.featuredImage?.asset?.metadata?.dimensions?.width ??
                "100%"
              }
              placeholder='blur'
              blurDataURL='data:image/jpeg;base64,'
            />
          </div>
          <SocialShareOverlay
            title={data?.name}
            // showShare='true'
            slug={`/specialties/${data?.slug?.current}`}
          />
        </a>
      </Link>
    );
  } else {
    return null;
  }
};

export default Img;
