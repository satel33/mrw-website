import Link from 'next/link'
import Image from 'next/image'
import Fade from 'react-reveal/Fade'
import { useRouter } from 'next/router'

// Assets
import PlaceholderImg from '../../../public/placeholder.jpeg'

const SearchResult = ({ title, slug, image, toggleSearch }) => {
  return (
    <Fade>
      <Link
        href={`${slug.includes('projects') ? slug : `/designs/${slug}`}`}
        prefetch={false}
      >
        <a className='text-left cursor-pointer block'>
          <div className='transition duration-150 shadow-lg hover:shadow-xl'>
            <Image
              objectFit='cover'
              src={image?.asset?.url ?? PlaceholderImg}
              layout='responsive'
              width={300}
              height={300}
              alt={image?.alt}
              placeholder='blur'
              blurDataURL='data:image/jpeg;base64,'
            />
          </div>
          <p className='font-medium block my-4 text-lg truncate transition duration-150 ease-in text-black-100 hover:text-primary-80'>
            {title}
          </p>
        </a>
      </Link>
    </Fade>
  )
}

export default SearchResult
