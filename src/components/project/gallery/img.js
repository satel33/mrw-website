...
// Components
const Caption = dynamic(() => import('../../../CMSComponents/shared/caption'), {
  ssr: false,
})
...
const Media = ({ img }) => {
  ...
  return (
    <>
      ...
      <div className='relative' onClick={() => openLightbox(true)}>
        <Image
          objectFit='cover'
          alt={img?.alt ?? ''}
          src={
        ...
        />
        {img?.imageCaption && <Caption data={img?.imageCaption} />}
      </div>
    </>
  )
}
