  ...
  return (
    <section className='max-w-screen-2xl mx-auto px-4'>
      {lightbox && image?.lightbox && (
        <Lightbox
          large={urlFor(image?.asset?._ref).quality(100).url()}
          onClose={(e) => openLightbox(false)}
          hideDownload={true}
          hideZoom={false}
          showRotate={true}
        />
      ...
      <div
        className='relative bg-gray-50 h-500 md:h-768'
        onClick={() => {
          openLightbox(true)
        }}
      >
        <Image
          alt={image?.alt ?? 'MRW Walls'}
          layout='fill'
          className='object-cover md:object-contain'
          src={
            image?.asset?._ref
              ? urlFor(image?.asset?._ref).quality(100).url()
              : PlaceholderImg
          }
          placeholder='blur'
          blurDataURL={
