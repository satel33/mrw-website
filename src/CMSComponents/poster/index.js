...
// Components
const Caption = dynamic(() => import('../shared/caption'), { ssr: false })
// Assets
  ...
  return (
    <>
      {lightbox && data?.lightbox && (
        <Lightbox
          large={urlFor(data?.asset?._ref).quality(100).url()}
      ...
      >
        <Image
          src={
            data?.asset?._ref ? urlFor(data?.asset?._ref).url() : PlaceholderImg
          }
          layout={data?.width && data?.height ? null : 'responsive'}
          width={data?.width ?? 500}
          height={data?.height ?? 460}
          alt={data?.alt}
          placeholder='blur'
          objectFit='cover'
          blurDataURL={
            data?.asset?._ref
              ? urlFor(data?.asset?._ref)
                  .width(data?.width ?? 500)
                  .height(data?.height ?? 460)
                  .blur(50)
                  .quality(10)
        ...
        />
        {data?.imageCaption && <Caption data={data?.imageCaption} />}
      </div>
    </>
  )
}
